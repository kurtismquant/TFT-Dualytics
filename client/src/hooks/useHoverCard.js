...
// Compact-size card box (UnitCard/ItemCard are 17.5rem wide); multiplied by the
// Interface Size factor since the cards are sized in rem.
const CARD_WIDTH = 280
const CARD_HEIGHT = 340
const LONG_PRESS_MS = 450
const MOVE_TOLERANCE_SQ = 100 // (10px)^2 — small jitter shouldn't cancel a long-press

function computePosition(rect, scale) {
  ...
  if (top + cardHeight > window.innerHeight - 8) {
    top = window.innerHeight - 8 - cardHeight
  }
...
export function useHoverCard(data, { delay = 300, touchTrigger = 'tap', anchorRef } = {}) {
  ...
  const onMouseEnter = useCallback((e) => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      if (!rect) return
      setPosition(computePosition(rect, scale))
      setIsOpen(true)
    }, delay)
  }, [isDragging, delay, anchorRef, scale])
